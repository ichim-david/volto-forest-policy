...
PROJECT_NAME=volto-forest-policy
STACK_NAME=volto-forest-policy-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://volto-forest-policy.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t ichim-david/volto-forest-policy-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t ichim-david/volto-forest-policy-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name volto-forest-policy-frontend-acceptance --link volto-forest-policy-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d ichim-david/volto-forest-policy-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name volto-forest-policy-backend-acceptance -d ichim-david/volto-forest-policy-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop volto-forest-policy-frontend-acceptance
	@docker stop volto-forest-policy-backend-acceptance
